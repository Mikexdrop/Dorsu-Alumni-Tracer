...
const MainBody = ({ onGetStarted }) => {
  ...
  useEffect(() => {
    ...
    async function fetchPosts() {
      try {
        const res = await fetch('http://dorsualumnitracer.com/api/posts/');
  ...
  React.useEffect(() => {
    ...
    async function fetchAlumniCount() {
      try {
        const res = await fetch('http://dorsualumnitracer.com/api/alumni/');
        const data = await res.json().catch(() => null);
        let count = null;
  ...
  React.useEffect(() => {
    ...
    async function fetchSurveysCount() {
      try {
        const res = await fetch('http://dorsualumnitracer.com/api/survey-aggregates/');
        if (!res.ok) return;
        const data = await res.json().catch(() => null);
        let cnt = null;
        if (data && typeof data === 'object') {
